...
## 2026-06 (cont.) — Persist Customer Completion Confirmation as legal evidence
- BACKEND: constants COMPLETION_CONFIRMATION_VERSION="2026-06-01" + COMPLETION_CONFIRMATION_TEXT. New endpoint POST /tasks/{task_id}/confirm-completion (client-only; admin allowed) → writes IMMUTABLE row to completion_confirmations {confirmation_id, booking_id, task_id, client_id, client_email, confirmation_version, confirmation_text, confirmed_at, ip (x-forwarded-for aware), user_agent} and flags booking+task (completion_confirmed, completion_confirmed_at, completion_confirmed_ip, completion_confirmation_version). Mirrors Terms acceptance evidence.
...
## 2026-06 — NEW: Ono-Fix Service Provider Agreement (separate provider consent)
- Providers now accept a SEPARATE "Service Provider Agreement" (independent-contractor agreement, v1.0) at registration, in addition to Terms of Use. Consent is tracked exactly like Terms (version + date/time + IP + User-Agent + SHA-256 document hash) with an immutable audit trail and admin PDF export.
- BACKEND (server.py): added `import hashlib`; `accepted_provider_agreement` on UserRegister; constants PROVIDER_AGREEMENT_VERSION="1.0", EFFECTIVE_DATE, full canonical text (49 sections, cleaned: placeholders filled — Platform Fee 15%, Restricted Period 12 months — and internal counsel notes removed) + PROVIDER_AGREEMENT_HASH (sha256). New endpoints: GET /provider-agreement/version (public), POST /provider-agreement/accept (provider/admin re-accept, client→403), GET /admin/users/{id}/provider-agreement-pdf (admin). register() now: providers MUST accept (else 400), stores accepted_provider_agreement_* fields on user + writes provider_agreement_acceptances audit row. Clients unaffected.
- FRONTEND: new app/provider-agreement.tsx (full 49-section agreement page, testid provider-agreement-screen); register.tsx shows a 2nd checkbox ONLY when role=provider (testid accept-provider-agreement-checkbox + provider-agreement-link), gates submit + Google signup; route added in _layout.tsx; users.tsx admin adds "Agreement PDF" button for providers (testid provider-agreement-pdf-btn-{id}); api.ts adminDownloadProviderAgreementPdf.
- DIVISION OF RESPONSIBILITY (per user): §30 reframed — Ono-Fix responsible for Platform/payment infra/its own actions & staff; Provider responsible for work/workmanship/licenses/permits/insurance/damage/workers/taxes/safety/invoice accuracy. §31 keeps the 6-month platform-fee liability cap.
- VERIFIED via curl (preview backend): version public; provider register w/o agreement→400, with→200; client register regression→200; acceptance fields+hash stored; admin PDF 200 application/pdf; re-accept ok; client re-accept→403. Frontend esbuild-clean; visual verification pending on Netlify after "Save to GitHub" (pod serves CRA stub, not Expo).
- server.py synced to /app/server.py.
- BACKLOG (deferred, per user's "would definitely add"): Provider → Documents section (W-9/tax, ID verification, License, Insurance, Background check, Payment verification uploads) + auto re-acceptance prompt when agreement version bumps (mechanism already supports version compare via GET /provider-agreement/version + accepted_provider_agreement_version on user).

## 2026-06 — NEW: Stripe Identity (provider KYC / identity verification)
- Providers must verify identity (government ID + selfie) via Stripe Identity (hosted redirect flow) BEFORE they are visible to clients and BEFORE they can accept jobs. User choice: mandatory gating (1a) + document+selfie (2a) + admin manual override + identity reflected in Service Provider Agreement PDF (3a).
- BACKEND (server.py): User model +identity fields (identity_verified, identity_status[unverified|processing|requires_input|verified], identity_verified_at, identity_verified_name, identity_verified_method[stripe|admin_manual], identity_last_error, stripe_verification_session_id). Endpoints: POST /identity/start (create/resume VerificationSession type=document + require_matching_selfie, returns hosted url; short-circuits if already verified), GET /identity/status (refreshes from Stripe if pending), POST /admin/users/{id}/verify-identity (manual override), POST /admin/users/{id}/unverify-identity (revoke). Webhook /webhook/stripe extended to handle identity.verification_session.{verified,processing,requires_input} via metadata.provider_id → _apply_identity_verified reads verified_outputs name. GATING: accept_task returns 403 until identity_verified; client provider-listing loop skips providers where identity_verified!=true. Agreement PDF (admin_provider_agreement_pdf) now prints an "Identity verification (Stripe Identity)" block (status/name/date/method).
- FRONTEND: app/identity.tsx (verify screen: steps + "Verify ID and selfie" → /identity/start → window.location redirect to Stripe), app/identity-complete.tsx (return_url landing, polls status), components/IdentityVerificationBanner.tsx (provider banner: verified chip / amber CTA → /identity), added to my-profile.tsx ProviderProfile header. Admin users.tsx: "Verify ID"/"Revoke ID" buttons + testids (verify-identity-btn-{id}, unverify-identity-btn-{id}). Routes registered in _layout.tsx (identity, identity-complete). api.ts: identityStart, identityStatus, adminVerifyIdentity, adminUnverifyIdentity.
- VERIFIED via curl (preview): status unverified→verified; admin verify/unverify ok; accept_task 403 when unverified, passes (404 missing task) when verified; agreement PDF 200 with identity block. Frontend esbuild-clean. NOT testable in pod: live Stripe hosted session — pod only has the Emergent proxy key `sk_test_emergent` which 401s against api.stripe.com; requires a REAL Stripe secret key (set in admin Integration Keys on prod) with Stripe Identity enabled + webhook events added. Backend reads key via _get_stripe_secret_key (DB integration keys → settings → env).
- ⚠️ BREAKING on deploy: mandatory gating hides ALL currently-unverified providers from clients and blocks them from accepting jobs. Admin must either have providers self-verify or use the new "Verify ID" button to grandfather existing trusted providers. (Seeded provider@handyhub.com left identity-verified in preview DB.)
- USER STRIPE DASHBOARD ACTIONS REQUIRED: enable Stripe Identity; add events identity.verification_session.verified/processing/requires_input to the existing Stripe webhook endpoint. Test mode does not perform real verification (simulated states only).
- server.py synced to /app/server.py.

## 2026-06 — Identity gating made toggle-controlled + Admin provider detail view
- FIX (bug: providers like "Vitalii Tutko" vanished from client "Pros" tab): the mandatory Stripe-Identity listing/accept gate now respects a new Settings flag `require_identity_verification` (default FALSE). When OFF, all providers are visible and can accept jobs (previous behavior restored). When ON, unverified providers are hidden from clients and get 403 on accept_task. Settings + SettingsUpdate gained the field; listing loop (~5818) and accept_task now check `settings.require_identity_verification`.
- ADMIN TOGGLE: Users screen header has a Switch "Require identity verification" (data-testid=toggle-require-identity) → reads api.getSettings() (GET /admin/settings), writes api.updateAdminSettings() (PUT /admin/settings/features). NOTE: plain PUT /admin/settings does NOT exist — use /admin/settings/features.
- ADMIN PROVIDER DETAIL VIEW: tapping a provider card (data-testid=user-card-{id}) opens a modal (data-testid=provider-detail-body) showing Service area (radius mi/km, cities, zones, zips), Pricing (hourly_rate, minimum_hours), Skills (chips from profile.skills, capitalize, per-skill rate if present), Availability (weekly + one-time slots from GET /admin/availability/{id}). Data via api.getExecutorProfile → GET /profile/executor/{id} and api.adminGetAvailability → GET /admin/availability/{id}. (Non-provider cards are not tappable.)
- VERIFIED via curl: toggle ON→accept 403, OFF→404; setting persists; /profile/executor/{id} & /admin/availability/{id} return expected shapes. esbuild-clean. Visible on live after Save to GitHub.
...
## 2026-06 — Provider onboarding guide (post-registration checklist)
- New providers land on a step-by-step setup guide after registration; a compact progress card stays on the provider home until all steps are done. Steps auto-detected from data (no manual ticking). English UI.
- BACKEND: GET /provider/onboarding-status → {steps:[{key,done}], completed, total, complete}. Keys: profile_photo (user.picture), work_zone (executor_profiles service_zones/cities/lat-lng/radius), skills (profile.skills>0), availability (availability_slots>=1), identity (user.identity_verified), notifications (user.telegram_chat_id). Provider/admin only. Verified via curl (4/6 for seeded provider).
